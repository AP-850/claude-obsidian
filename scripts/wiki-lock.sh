...
validate_path() {
  ...
  if command -v python3 >/dev/null 2>&1; then
    local resolved root
    # `|| resolved=""` matters here: on some systems `python3` on PATH is a
    # non-functional stub (e.g. the Windows Store "app execution alias") that
    # exits non-zero without running any code. Under `set -e` an unguarded
    # command substitution assignment would abort the whole script on that
    # exit status; falling back to empty output reproduces the same
    # skip-the-check behaviour as the "no python3 available" branch below.

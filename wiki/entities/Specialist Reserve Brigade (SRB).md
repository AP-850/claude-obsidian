---
type: entity
title: "Specialist Reserve Brigade (SRB)"
address: c-000030
aliases:
  - SRB
created: 2026-08-05
updated: 2026-08-05
tags:
  - entity
  - pg-unit
  - pg2030
  - reserve
status: active
related:
  - "[[Programme MAKHLAB]]"
  - "[[Falcons Brigade]]"
  - "[[PG 2030 Force Design Concepts]]"
  - "[[sources/updated-pg-2030-tasks-aug-2026]]"
  - "[[sbtdws/_index]]"
---

# Specialist Reserve Brigade (SRB)

Reserve component for PG Regular Brigades, held at readiness ("Wolf teams") to reinforce for Military Response Contingencies (MRC) 2 and 4. Pivoting toward a UxS National Resistance role.

Navigation: [[entities/_index]] | [[sbtdws/_index]] | [[Programme MAKHLAB]] | [[PG 2030 Force Design Concepts]]

---

## Role

| Field | Detail |
|-------|--------|
| Current role/focus | Wolf teams at readiness to reinforce Regular PG Bdes for MRC 2 & 4 |
| Revised PG 2030 role/focus | UxS National Resistance capability & maximising civilian expertise |
| Force Design Concept | 8 — Uncrewed Systems & Lethality Concept (Programme MAKHLAB) |

---

## Develop by 2030

1. UxS to support National Resistance and provide second-echelon support to [[Falcons Brigade]]
2. Mechanisms for maximising Reserve personnel civilian skill sets

---

## Tasks (Aug 2026 register)

1. Develop an SRB Concept of Use (CONUSE) to provide terms of reference moving forward
2. **MAKHLAB 4** — develop and integrate AI-enabled Uncrewed Systems (UxS) for enhanced PG National Resistance capability
   - A. Enhance the Brigade's ability to provide UxS support and reinforcement to [[Falcons Brigade]] for MRC 3
   - B. Develop capability for reservists to remotely operate UxS from the home base
3. Identify and develop initiatives to capitalize on the civilian expertise of its Reservist personnel
4. Sustain ability to deliver in-house training for the PG Reserve Cadre

> [!note] Two original tasks compressed out of the UPDATED numbering
> The original (pre-UPDATE) task list also carried "Develop revised Reserve terms and conditions of service to enable personnel to be routinely mobilized in peacetime — a J1/J3 task" as its own numbered item. The UPDATED list folds this into the surrounding tasks rather than dropping the requirement outright — worth re-confirming with J1/J3 whether this reserve-mobilization work is still active.

---

## Source

[[PG 2030 Force Design Concepts]], [[sources/updated-pg-2030-tasks-aug-2026]] | `.raw/PG 2030 New/Updated PG 2030 Tasks_Aug 2026.pdf`
